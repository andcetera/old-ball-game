...
public class GameScreen implements Screen, InputProcessor, GestureListener {
	...
	public void render(float delta) { 
		...
		if(Gdx.input.isKeyJustPressed(Keys.BACK)){//still need, not working w/input processor
			pause = true;
		...
		batch.begin();
			...
			if(tilt){
				fBounds = fireFont.getBounds("Tilt!");
				fireFont.draw(batch, "Tilt!", edges.x/2 - fBounds.width/2, edges.y/2 - fBounds.height);
			}
		batch.end();
	...
	public void generalUpdate(Vector3 touch, OrthographicCamera camera) {
		...
		//Paddle
		if(Gdx.app.getType() == ApplicationType.Android && Math.abs(Gdx.input.getAccelerometerX()) > 0.3){
			paddle.bounds.x -= Gdx.input.getAccelerometerX() * Gdx.graphics.getDeltaTime() * GameMain.SENSE;
		}
		if (moveL){//for desktop, +UP for fire, etc?...
			paddle.bounds.x -= 20;
		} else if (moveR){
			paddle.bounds.x += 20;
		}
		...
		if(Math.abs(ball.speed.y/ball.speed.x) >= tilted) tilt = false;
	...
	//ARE THE RETURN TRUE;S NEEDED IN THESE??
	@Override
	...
	public boolean keyUp(int keycode) {
		...
		if(keycode == Keys.UP){
			...
			if(!pause && Math.abs(ball.speed.y/ball.speed.x) < tilted){
				tilt = true;
			}
		}
		return false;
